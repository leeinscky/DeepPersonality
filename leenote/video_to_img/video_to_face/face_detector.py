...
class FaceDetection:
    ...
    def find_face(self, img):
        ret = True
        self.rects = self.detector(img, 1) # # detect input img, para 1 means 1 times upsamle to detect small faces (default is 0)
        # 经过测试发现：第二个参数设为0时，Expert 和 Novice 检测到的人脸数量不一致，例如ls 003/Expert_video/ | wc -l 为88，ls 003/Novice_video/ | wc -l 为77; 但是设为1时，两者一致，都为83
        if len(self.rects) == 0:
