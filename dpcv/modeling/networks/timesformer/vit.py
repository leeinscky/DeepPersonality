...
# modified from https://github.com/facebookresearch/TimeSformer
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        # print('[timesformer] VisionTransformer.forward_features函数, 输入x:', x.shape)
        B = x.shape[0]
        ...
        x = self.norm(x)
        # print('[timesformer] VisionTransformer.forward_features函数, after norm, x:', x.shape, ', final return x[:, 0]:', x[:, 0].shape)
        return x[:, 0]

    def forward(self, x):
        # print('[timesformer] 1-VisionTransformer.forward函数, 输入x:', x.shape)
        x = self.forward_features(x)
        # print('[timesformer] 2-VisionTransformer.forward函数 - after forward_features, x shape:', x.shape)
        x = self.head(x)
        # print('[timesformer] 3-VisionTransformer.forward函数 - after head, x shape:', x.shape)
...
class vit_base_patch16_224(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.model = VisionTransformer(img_size=cfg.DATA.TRAIN_CROP_SIZE, num_classes=cfg.MODEL.NUM_CLASSES, patch_size=patch_size,
                                       in_chans=6, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
                                       norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0., attn_drop_rate=0.,
                                       drop_path_rate=0.1, num_frames=cfg.DATA.NUM_FRAMES,
    ...
    def __init__(self, img_size=224, patch_size=16, num_classes=400, num_frames=8, attention_type='divided_space_time',  pretrained_model='', **kwargs):
        ...
        if self.pretrained:
            load_pretrained(self.model, num_classes=self.model.num_classes, in_chans=kwargs.get('in_chans', 3),
                            filter_fn=_conv_filter, img_size=img_size, num_frames=num_frames, num_patches=self.num_patches,
...
def timesformer_udiva(cfg):
    model = TimeSformer(img_size=224, num_classes=2, num_frames=16, attention_type='divided_space_time',  
...
if __name__ == "__main__":
    model = TimeSformer(img_size=224, num_classes=400, num_frames=8, attention_type='divided_space_time',
                        pretrained_model='pretrained/TimeSformer_divST_16x16_448_K400.pyth')
