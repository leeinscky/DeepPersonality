...
def load_pretrained(model, cfg=None, num_classes=1000, in_chans=3, filter_fn=None, img_size=224, num_frames=8, num_patches=196, attention_type='divided_space_time', pretrained_model="", strict=True):
    ...
    # 解决报错：size mismatch for patch_embed.proj.weight: copying a param with shape torch.Size([768, 3, 16, 16]) from checkpoint, the shape in current model is torch.Size([768, 6, 16, 16]).
    # 由于预训练模型的输入通道数为3，而当前模型的输入通道数为6，因此需要将预训练模型的输入通道数改为6， 通过cat的方式将预训练模型的输入通道数改为6
    state_dict['patch_embed.proj.weight'] = torch.cat((state_dict['patch_embed.proj.weight'], state_dict['patch_embed.proj.weight']), dim=1)
    # print('state_dict[patch_embed.proj.weight].shape: ', state_dict['patch_embed.proj.weight'].shape)
