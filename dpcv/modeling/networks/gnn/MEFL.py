...
class GraphModel(nn.Module):
    def __init__(self, init_weights=True, num_class=2):
        super(GraphModel, self).__init__()

        if init_weights:
            initialize_weights(self)
        
        self.graph_model = MEFARG(num_classes=num_class, backbone='resnet50')

    def forward(self, x):
        print('[MEFL.py] GraphModel.forward, input x.shape: ', x.shape) # [bs, sample_size, 3*2=6, 224, 224]
        # 遍历每个sample_size
        for i in range(x.shape[1]):
            one_sample_feature =  x[:, i, :, :, :]
            one_person_face = one_sample_feature[:, 0:3, :, :]
            print('[MEFL.py] GraphModel.forward, one_sample_feature.shape: ', one_sample_feature.shape, ', one_person_face.shape:', one_person_face.shape) # [bs, 3*2=6, 224, 224]
            cl, cl_edge = self.graph_model(one_person_face)
            print('[MEFL.py] GraphModel.forward, cl.shape: ', cl.shape) # [bs, num_class]
        return cl


@NETWORK_REGISTRY.register()
def visual_graph_representation_learning(cfg=None): # 视觉图表示学习
    graph_model = GraphModel(num_class=cfg.MODEL.NUM_CLASS)
