...
class CrossAttn(nn.Module):
    ...
    def forward(self, y, x):
        ...
        """
        # print('[graph_edge_model.py] CrossAttn, input y.shape: ', y.shape, ', x.shape: ', x.shape, 'y.dtype: ', y.dtype, ', x.dtype: ', x.dtype)
        if torch.cuda.is_available() and use_half:
            y = y.half()
            x = x.half()
        # print('[graph_edge_model.py] CrossAttn, linear_q.dtype: ', next(self.linear_q.parameters()).dtype, ', linear_k.dtype: ', next(self.linear_k.parameters()).dtype, ', linear_v.dtype: ', next(self.linear_v.parameters()).dtype)
...
class GEM(nn.Module):
    ...
    def forward(self, class_feature, global_feature):
        ...
        if self.num_classes != 15: # 12 or 8
