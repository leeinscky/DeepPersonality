...
class AudioVisualResNet18Udiva(nn.Module): # nn.Module是所有神经网络模块的基类，你的网络也应该继承这个类，这样可以方便地使用各种神经网络模块，比如卷积、全连接、激活函数等等，这些模块都是nn.Module的子类。你可以直接调用，也可以自己定义，然后继承nn.Module，这样就可以使用了，这样做的好处是可以方便地组合各种模块，构建复杂的网络结构，而且可以方便地调用各种模块的参数，比如权重、偏置等等。
    def __init__(self, init_weights=True, return_feat=False):
...
@NETWORK_REGISTRY.register()
def audiovisual_resnet_udiva(cfg=None):
